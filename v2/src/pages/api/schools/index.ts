...
import { createSchoolRecord, listSchoolsForTenant } from '~/lib/school-api-store';
import { resolveTenantContext } from '~/lib/tenant-context';
import { SchoolCreateInput } from '~/schemas/school-api';
...
export const GET: APIRoute = async (context) => {
  ...
  const result = await listSchoolsForTenant(context.locals.runtime.env.DB, tenant.tenant, {
    limit,
    offset,
...
export const POST: APIRoute = async (context) => {
  ...
  const parsed = SchoolCreateInput.safeParse(raw);
  if (!parsed.success) return apiError(422, 'schema_invalid', parsed.error.issues);

  const result = await createSchoolRecord(context.locals.runtime.env.DB, tenant.tenant, parsed.data);
  if (!result.ok) return apiError(result.status, result.reason, result.detail);

  return noStore(json(201, { ok: true, tenant: tenant.tenant, school: result.record }));
};
