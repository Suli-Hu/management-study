/**
 * Scholar 直写 D1 (v0.5.93)
 *
 * 写入边界：
 *   1. scholar 主表 (UPSERT)
 *   2. scholar_school join — 重建 schools[] 声明部分（position 0..N）
 *      KP-driven fallback rows（position 1000）保持不动
 *   3. kp_scholar join — 按 kpsOrder[] 重建 scholar-driven 部分（position 0..N）
...
export async function upsertScholarInD1(
  ...
  // 1. scholar 主表 UPSERT（accent 列废弃，写空字符串与 sync 脚本一致）
  stmts.push(
    db.prepare(
      `INSERT INTO scholar (
  ...
  stmts.push(
    db.prepare('DELETE FROM scholar_school WHERE scholar_key = ? AND position < 1000')
      .bind(scholar.key),
  );
  scholar.schools.forEach((schoolKey, position) => {
    stmts.push(
      db.prepare(
        `INSERT OR IGNORE INTO scholar_school (scholar_key, school_key, position)
         VALUES (?, ?, ?)`,
      ).bind(scholar.key, schoolKey, position),
    );
  });

  // 3. kp_scholar join — 按 kpsOrder[] 重建 scholar-driven 部分 (position < 1000)
  stmts.push(
    db.prepare('DELETE FROM kp_scholar WHERE scholar_key = ? AND position < 1000')
      .bind(scholar.key),
  );
  scholar.kpsOrder.forEach((kpId, position) => {
    stmts.push(
      db.prepare(
        `INSERT OR IGNORE INTO kp_scholar (kp_id, scholar_key, position)
         VALUES (?, ?, ?)`,
      ).bind(kpId, scholar.key, position),
    );
  });
...
export async function deleteScholarInD1(
  ...
  await db.batch([
    db.prepare('DELETE FROM kp_scholar WHERE scholar_key = ?').bind(scholarKey),
    db.prepare('DELETE FROM scholar_school WHERE scholar_key = ?').bind(scholarKey),
    db.prepare('DELETE FROM scholar WHERE key = ?').bind(scholarKey),
  ]);
}
