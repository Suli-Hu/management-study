 ...
 * 与学派 accent 无关，是固定的语义色 tone。
 *
 * v0.5.46 配色调整为 Z 三组色（之前 Apple system 6 色"花里胡哨"）：
 *   - 积极组（义/应/用）= 绿 #10B981 — 核心价值 + 行动策略
 *   - 警告组（限）     = 红 #EF4444 — 边界/风险
 *   - 中性组（例/喻）   = 灰 #6B7280 — 实例与隐喻 (辅助拓展)
 *
 * - glyph: 单字（也是 eval_content_json 的 key）
 * - name: 中文全称（UI 标签 — 渲染时用 pill 形 tone 色显示）
...
export const EVAL_TAG_DEFS: readonly EvalTagDef[] = [
  { glyph: '义', name: '意义', matches: ['意义', '義'],                     tone: '#10B981' },
  { glyph: '限', name: '局限', matches: ['局限', '周限', '限界', '限'],     tone: '#EF4444' },
  { glyph: '例', name: '例子', matches: ['例子', '事例', '例'],             tone: '#6B7280' },
  { glyph: '应', name: '应对', matches: ['应对', '応対', '応'],             tone: '#10B981' },
  { glyph: '用', name: '应用', matches: ['应用', '応用', '用'],             tone: '#10B981' },
  { glyph: '喻', name: '比喻', matches: ['比喻', '喩'],                     tone: '#6B7280' },
] as const;
