...
describe('patchKpsBatch — 真 SQLite 集成测试', () => {
  ...
  test('forbidden_field: patch 含 id → forbidden_field + 返 current_version', async () => {
    ...
    const rawUpdates = [{ id: 'k030', ifMatchVersion: 1, patch: { id: 'kHACK' } }];
    const out = await patchKpsBatch(
      db as unknown as D1Database,
      updates,
      { dryRun: false, tenant: TENANT, userId: USER_ID },
  ...
  test('shallow merge body: 只传 zh，ja 保留', async () => {
    await seedKp(db, 'k060', { body: { zh: '旧正文', ja: '旧本文' } });
    const updates = [
      { id: 'k060', ifMatchVersion: 1, patch: { body: { zh: '新正文' } } },
    ];
    await patchKpsBatch(
      db as unknown as D1Database,
      updates,
  ...
  test('shallow merge evalContent: 只传 zh，ja 保留；但 zh 内部 Record 整体替换', async () => {
    ...
    const updates = [
      {
        id: 'k070',
        ifMatchVersion: 1,
        patch: { evalContent: { zh: { 义: '新义', 应: '新应' } } },
      },
    ];
    await patchKpsBatch(
      db as unknown as D1Database,
    ...
    const row = await getKpRow(db, 'k070');
    const evalZh = JSON.parse(row!.eval_content_zh_json);
    const evalJa = JSON.parse(row!.eval_content_ja_json);
    // zh Record 整体替换：旧 限/例 没了
    expect(evalZh).toEqual({ 义: '新义', 应: '新应' });
    // ja 保留
    expect(evalJa).toEqual({ 義: '旧義' });
  });
  ...
  test('dryRun + invalid (forbidden_field): 仍返 current_version', async () => {
    ...
    const rawUpdates = [{ id: 'k110', patch: { id: 'kHACK' } }];
  ...
  test('mergeBatchPatch unit: 数组 [] = 真清空，不传 = 保持', () => {
    ...
    const cleared = mergeBatchPatch(current, { tags: [] });
    expect(cleared.tags).toEqual([]); // 空数组真清空

    const kept = mergeBatchPatch(current, { year: '2020' });
    expect(kept.tags).toEqual(['old']); // 不传 = 保持
  });
